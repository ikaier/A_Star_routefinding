...
class HuskyMapper:
    ...
    def from_laser_to_map_coordinates(self, points_in_baselaser_frame):
        ...
        for i in range(len(points_in_baselaser_frame)):
            points_in_map_frame[i] = np.add(points_in_baselaser_frame[i], np.array([self.baselaser_x_in_map, self.baselaser_y_in_map, 0]))
    ...
    def process_laser_scan(self, ranges_in_baselaser_frame):
        ...
        for i in range(len(ranges_in_baselaser_frame)):
            ...
            points_xyz_in_baselaser_frame.append(
                [np.array([range_of_i * cos(angle_of_i), range_of_i * sin(angle_of_i), 0])])
