...
class HuskyMapper:
    ...
    def from_laser_to_map_coordinates(self, points_in_baselaser_frame):
        ...
        #add map coordinate of robot and the coordinate of point in robot frame => coordinates of point in map frame
        points_in_map_frame = [np.add(np.array([self.baselaser_x_in_map, self.baselaser_y_in_map, 0]), xyz_baselaser)
    ...
    def is_in_field_of_view(self, robot_row, robot_col, laser_theta, row, col):
        ...
        robot_point=np.array([robot_col,robot_row])
        point=np.array([col,row])
        dist = np.linalg.norm(robot_point - point)*self.ogm.meters_per_cell


        # calculate angle of the cell
        # check angle difference with laser_theta
        dist_of_y = (row - robot_row) * self.ogm.meters_per_cell
        dist_of_x = (col - robot_col) * self.ogm.meters_per_cell
        angle = atan2(dist_of_y, dist_of_x)
        angle_difference = atan2(sin(angle - laser_theta), cos(angle - laser_theta))
        #return true if all requirement satisfied
        return self.max_laser_range > dist > self.min_laser_range and self.min_laser_angle < angle_difference < self.max_laser_angle
    ...
    def inverse_measurement_model(self, row, col, robot_row, robot_col, robot_theta_in_map, beam_ranges, beam_angles):
        ...
        robot_point = np.array([robot_col, robot_row])
        point = np.array([col, row])
        r = np.linalg.norm(robot_point - point) * self.ogm.meters_per_cell
    ...
    def process_laser_scan(self, ranges_in_baselaser_frame):
        ...
        for i in range(len(ranges_in_baselaser_frame)):
            ...
            angle = self.angles_in_baselaser_frame[i]
            #if r is in the map range, add the point to the list, otherwise ignroe it.
            if self.max_laser_range > r > self.min_laser_range:
                #since the robot rotates, we need to calculate the actual location of the point in map frame by using the rotate angle
                points_xyz_in_baselaser_frame.append(np.array([r * cos(angle), r * sin(angle), 0]))  # CHANGE THIS
